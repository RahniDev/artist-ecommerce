...
export const getCategory = async (req, res) => {
    try {
        const subcategories = await Category.find({ parent: req.category._id })
            .select('_id name')
            .lean();
        const subcategoriesWithProducts = await Promise.all(subcategories.map(async (sub) => {
 ...
 */
export const listNested = async (req, res) => {
    try {
        ...
        const tree = topLevel.map((parent) => ({
            ...parent,
            subcategories: all.filter((c) => c.parent && c.parent.toString() === parent._id.toString()),
        }));
